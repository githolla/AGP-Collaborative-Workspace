/**
 * The Kantata pull, extracted from api/mirror.ts so it has exactly one
 * implementation shared by two callers: api/mirror.ts's own HTTP handler
 * (unchanged behavior — same caching, same query params, same response
 * shape) and the new Kantata-import endpoints (api/account-import.ts,
 * api/account-deepen.ts), which call these functions in-process rather
 * than duplicating the fetch/pagination/rate-limit logic a second time.
 *
 * KANTATA_API_TOKEN is read by the CALLER and passed in here — this file
 * has no env access of its own, so it stays a pure "given a token, pull
 * this data" module regardless of which endpoint is asking.
 *
 * Self-contained per this repo's api/*.ts convention (no workspace
 * imports) — Vercel bundles each function independently of the pnpm
 * monorepo, and api/_lib is the one exception to "one file, no imports"
 * that convention already allows (see db.ts, requireUser.ts, etc.).
 */
...
export async function pullKantata(token: string): Promise<KantataPull> {
...
export interface KantataFocusPull {
  milestones: Record<string, unknown>[];
  tasks: Record<string, unknown>[];
  note: string;
}
 ...
 */
export async function pullWorkspaceStories(token: string, workspaceIds: string[]): Promise<KantataFocusPull> {
  const headers = { Authorization: `Bearer ${token}` };
...
export interface WorkspaceTitle {
  id: string;
  title: string;
}

/**
 * Just the title for specific workspaces — one direct GET per id (Mavenlink
 * has no bulk-by-id filter on this collection), in parallel, same shape as
 * `pullWorkspaceStories`'s per-workspace walk. Built for
 * `api/_lib/msFolder.ts`'s folder-tree computation (teams-provisioning-plan.md
 * B3/B4): it needs the project-level (workspace) name, which the stories
 * endpoint alone doesn't carry. A workspace that 404s or errors is simply
 * absent from the result — the caller treats a missing project the same way
 * it already treats one dropped from `mirror.kantata_workspaces` (reported,
 * never guessed).
 *
 * Response shape is the same id-keyed map every Mavenlink collection uses
 * (`{ workspaces: { "<id>": {...} } }`), not a singular `{ workspace: {...} }`
 * — verified against a live call before trusting it, since guessing this
 * wrong silently returns zero titles rather than an error.
 */
export async function pullWorkspaceTitles(token: string, workspaceIds: readonly string[]): Promise<WorkspaceTitle[]> {
  const headers = { Authorization: `Bearer ${token}` };
  const results = await Promise.allSettled(
    workspaceIds.map(async (id): Promise<WorkspaceTitle | null> => {
      const res = await timedFetch(`https://api.mavenlink.com/api/v1/workspaces/${id}`, headers);
      if (!res.ok) return null;
      const json = (await res.json()) as { workspaces?: Record<string, { title?: string }> };
      const title = json.workspaces?.[id]?.title;
      return title ? { id, title } : null;
    }),
  );
  return results.filter((r): r is PromiseFulfilledResult<WorkspaceTitle> => r.status === "fulfilled" && r.value !== null).map((r) => r.value);
}
...
/**
 * The AGP team roster with emails — `GET /account_memberships?include=user`,
 * the same single endpoint `pullKantata`'s own `harvestStaff` reads (see
 * that function's comment), factored out standalone here because a full
 * tenant-wide pull is far more than resolving emails for
 * `collab.account_member` needs (teams-provisioning-plan.md B5: adding
 * someone to the Microsoft Team needs an email, and most Kantata-seeded
 * members never had one recorded — see migration 0016).
 */
export async function pullStaffRoster(token: string): Promise<StaffMember[]> {
  const headers = { Authorization: `Bearer ${token}` };
  const staffById = new Map<string, StaffMember>();
  for (let page = 1; page <= 5; page += 1) {
    const res = await timedFetch(`https://api.mavenlink.com/api/v1/account_memberships?per_page=200&include=user&page=${page}`, headers);
    if (!res.ok) {
      if (page === 1) throw new Error(`account_memberships HTTP ${res.status}`);
      break;
    }
    const json = (await res.json()) as { account_memberships?: Record<string, unknown>; users?: Record<string, { full_name?: string; headline?: string; email_address?: string }> };
    const users = json.users ?? {};
    for (const [id, u] of Object.entries(users)) {
      if (u?.full_name) staffById.set(String(id), { id: String(id), name: u.full_name, title: u.headline ?? "", email: u.email_address ?? "" });
    }
    const batch = Object.values(json.account_memberships ?? {});
    if (batch.length < 200) break;
  }
